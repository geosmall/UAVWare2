...
/* define config file name and id for persistent storage */
#define CONFIG_FILE_NAME    "./config.bin"
#define V0p1_SIGNATURE      0x45

static int config_save_to_file( void );
...
int config_load_from_file( void )
{
  ...
  file_size = UW_fs_file_size( CONFIG_FILE_NAME );
  if ( file_size < 0 ) {
    /* Error getting config file size */
    goto restore_defaults_save_and_exit;
  }

  /* check if config file size matches struct size */
  if ( file_size == sizeof( config ) ) {
    /* Go ahead and read flash data into config struct */
    result = UW_fs_read_file( CONFIG_FILE_NAME, ( uint8_t * )&config, sizeof( config ) );
    if ( result < 0 ) {
      goto restore_defaults_save_and_exit;
    }
  }

  /* See if we know what to do with the current config data
   * config.version holds the magic number from struct */
  if ( config.version == V0p1_SIGNATURE ) {
    /* if we get to here, correct size read in, version signature matches,
       so assume config data valid and return */
    UVOS_COM_SendString( UVOS_COM_DEBUG, "Config file successful read from file system.\r\n" );
    return CFG_ERR_OK;
  }

  /* Unknown version or error - restore to factory defaults
     Note: this removes any existing config file */
restore_defaults_save_and_exit:
  UVOS_COM_SendString( UVOS_COM_DEBUG, "ERROR: Config file defaults restored and saved.\r\n" );
  /* Load factory defaults */
  config_set_defaults();
  /* Save back to config file */
  if ( config_save_to_file() < 0 ) {
    return CFG_ERR_FAILED;
  } else {
    return CFG_ERR_OK;
  }
...
void config_print_file( void )
{
  UW_fs_dir( "./" );
  UW_fs_print_hex_file( CONFIG_FILE_NAME );
}
...
static int config_save_to_file( void )
{
  ...
  return ( UW_fs_write_file( CONFIG_FILE_NAME, ( uint8_t * )&config, sizeof( config ) ) );
}

static void config_set_defaults( void )
{
  config.version = V0p1_SIGNATURE;
...
//========================================================================================================================//
  ...
  /* Magnetometer calibration parameters - if using MPU9250, uncomment calibrateMagnetometer() in setup() to get these values, else just ignore these */
