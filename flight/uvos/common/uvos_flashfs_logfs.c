...
enum uvos_flashfs_logfs_dev_magic {
  UVOS_FLASHFS_LOGFS_DEV_MAGIC = 0x94938201,
};

struct logfs_state {
  enum uvos_flashfs_logfs_dev_magic magic;
  ...
  const struct uvos_flash_driver * driver;
  uintptr_t flash_id;
};
...
{
  UVOS_Assert( arena_id < ( logfs->cfg->total_fs_size / logfs->cfg->arena_size ) );
  UVOS_Assert( slot_id < ( logfs->cfg->arena_size / logfs->cfg->slot_size ) );
...
static bool UVOS_FLASHFS_Logfs_validate( const struct logfs_state * logfs )
{
  return logfs && ( logfs->magic == UVOS_FLASHFS_LOGFS_DEV_MAGIC );
}

#if defined(UVOS_INCLUDE_FREERTOS)
static struct logfs_state * UVOS_FLASHFS_Logfs_alloc( void )
{
  struct logfs_state * logfs;

  logfs = ( struct logfs_state * ) UVOS_malloc( sizeof( *logfs ) );
  if ( !logfs ) {
    return NULL;
  }

  logfs->magic = UVOS_FLASHFS_LOGFS_DEV_MAGIC;
  return logfs;
}
static void UVOS_FLASHFS_Logfs_free( struct logfs_state * logfs )
{
...
#else
static struct logfs_state uvos_flashfs_logfs_devs[UVOS_FLASHFS_LOGFS_MAX_DEVS];
static uint8_t uvos_flashfs_logfs_num_devs;
static struct logfs_state * UVOS_FLASHFS_Logfs_alloc( void )
{
  struct logfs_state * logfs;

  if ( uvos_flashfs_logfs_num_devs >= UVOS_FLASHFS_LOGFS_MAX_DEVS ) {
    return NULL;
  }

  logfs = &uvos_flashfs_logfs_devs[uvos_flashfs_logfs_num_devs++];
  logfs->magic = UVOS_FLASHFS_LOGFS_DEV_MAGIC;
...
int32_t UVOS_FLASHFS_Logfs_Init( uintptr_t * fs_id, const struct flashfs_logfs_cfg * cfg, const struct uvos_flash_driver * driver, uintptr_t flash_id )
{
  ...
  UVOS_Assert( ( cfg->total_fs_size / cfg->arena_size > 1 ) );

  /* Make sure the underlying flash driver provides the minimal set of required methods */
  UVOS_Assert( driver->start_transaction );
  UVOS_Assert( driver->end_transaction );
  UVOS_Assert( driver->erase_sector );
  UVOS_Assert( driver->write_data );
  UVOS_Assert( driver->read_data );
  ...
  logfs = ( struct logfs_state * )UVOS_FLASHFS_Logfs_alloc();
  if ( logfs ) {
    while ( rc && count++ < 2 ) {
      ...
      if ( logfs->driver->start_transaction( logfs->flash_id ) == 0 ) {
        ...
        if ( rc != 0 ) {
          ...
          UVOS_FLASHFS_Format( ( uintptr_t )logfs );
          rc -= 3;
          goto out_end_trans;
        }
...
int32_t UVOS_FLASHFS_Logfs_Destroy( uintptr_t fs_id )
{
  ...
  if ( !UVOS_FLASHFS_Logfs_validate( logfs ) ) {
    rc = -1;
    goto out_exit;
  }

  UVOS_FLASHFS_Logfs_free( logfs );
  rc = 0;
...
{
  UVOS_Assert( slot_hdr );
  UVOS_Assert( curr_slot );
...
static int8_t logfs_reserve_free_slot( struct logfs_state * logfs, uint16_t * slot_id, struct slot_header * slot_hdr, uint32_t obj_id, uint16_t obj_inst_id, uint16_t obj_size )
{
  ...
  UVOS_Assert( candidate_slot_id > 0 );
...
/**********************************
 *
 * Provide a UVOS_FLASHFS_* driver
 *
 *********************************/
#include "uvos_flashfs.h" /* API for flash filesystem */
...
int32_t UVOS_FLASHFS_ObjSave( uintptr_t fs_id, uint32_t obj_id, uint16_t obj_inst_id, uint8_t * obj_data, uint16_t obj_size )
{
  ...
  UVOS_Assert( obj_size <= ( logfs->cfg->slot_size - sizeof( struct slot_header ) ) );
